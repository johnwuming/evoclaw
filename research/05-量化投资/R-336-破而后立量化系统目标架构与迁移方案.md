...
- **动作**：
  1. portfolio_version 对象落地（R-335 M1 的 vC-0 快照；首条=当前在役三元组 A:a13_rsraw_e1f10dz + gold:active_paper + ddc 0.20/0.5/0.05 + 权重口径）
...
| canary（预留） | Canary Stage | 小资金灰度（当前未启用） |
| ddc 状态机 | Drawdown Controller（DDC） | sleeve 级风控（≤−20%×0.5，回补 −5%） |
