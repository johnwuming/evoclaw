#!/usr/bin/env python3
# task-0606: 从已落盘数据生成 R-391 报告（表格程序化生成，防手误）
import pandas as pd, json, numpy as np

W = "/root/.openclaw/workspace/shared/results/work"
OUT = "/root/.openclaw/workspace/shared/results/05-量化投资/R-391-gold引擎月末NaN缺陷审计与修复方案.md"

sig = pd.read_csv(f"{W}/task-0606-hp-signals.csv", index_col=0, parse_dates=[0])
j = pd.read_csv(f"{W}/task-0606-ledger-join.csv", index_col=0, parse_dates=[0])
cont = pd.read_csv(f"{W}/task-0606-contaminated.csv", index_col=0, parse_dates=[0])
res = json.load(open(f"{W}/task-0606-results.json"))
disp = json.load(open(f"{W}/task-0606-display-impact.json"))

# --- 表1：61 个 NaN 月末全清单（热身10 + 日历51） ---
nan_m = sig[sig["sma_eng"].isna()]
rows1 = []
for d, r in nan_m.iterrows():
    typ = "热身" if r["is_warmup_nan"] else "日历NaN"
    dis = "是" if r["w_disagree"] else "否(两侧同0)"
    sma_s = "NaN(不足200日)" if pd.isna(r["sma_asof"]) else f"{r['sma_asof']:.4f}"
    rows1.append(f"| {d.date} | {r['px']:.4f} | {sma_s} | 0.0000 | {r['w_asof']:.4f} | {typ} | {dis} |")
table1 = "\n".join(rows1)

# --- 表2：33 个污染账本行 ---
rows2 = []
for d, r in cont.iterrows():
    tag = "错过涨幅" if r["pnl_diff_gross"] > 0 else "侥幸避跌"
    rows2.append(f"| {d.date} | 0.0000 | {r['w_true']:.4f} | {r['gold_ret']*100:+.2f}% | {r['pnl_diff_gross']*100:+.2f}pp | {tag} |")
table2 = "\n".join(rows2)

g, t = disp["gold_leg_actual_157m"], disp["gold_leg_true_157m"]
pa, pt = disp["display5842_actual_156m"], disp["display5842_true_156m"]

report = f"""# R-391 gold 引擎「月末 NaN→w=0」缺陷审计与修复方案
...
|---|---|---|---|
| 终点净值 | {g['end']:.4f} | {t['end']:.4f} | **+{t['end']-g['end']:.4f}（真值高 {100*(t['end']/g['end']-1):.1f}%）** |
| 年化收益 | {100*g['ann']:.2f}% | {100*t['ann']:.2f}% | **+{100*(t['ann']-g['ann']):.2f}pp** |
| 最大回撤 | {100*g['mdd']:.2f}%（{g['mdd_when']}） | {100*t['mdd']:.2f}%（{t['mdd_when']}） | **深 {100*(t['mdd']-g['mdd']):.2f}pp** |
...
|---|---|---|
| 组合终点（156 月） | {pa['end']:.2f} | {pt['end']:.2f}（+{100*(pt['end']/pa['end']-1):.1f}%） |
| 组合年化 | {100*pa['ann']:.2f}% | {100*pt['ann']:.2f}%（+{100*(pt['ann']-pa['ann']):.2f}pp） |
| 组合 MDD | {100*pa['mdd']:.2f}%（{pa['mdd_when']}） | {100*pt['mdd']:.2f}%（{pt['mdd_when']}，持平——最深回撤由 A 腿 2015 股灾主导） |
