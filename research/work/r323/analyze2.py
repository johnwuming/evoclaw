...
def daily_feats(code):
    d=pd.read_csv(f"{R}/raw/{code}_sina.csv",parse_dates=["date"]).sort_values("date")
...
M={c:monthly_close(f"{c}_sina.csv") for c in codes}
...
json.dump({"signals":out,"vs_a13":a13vals,"vs_gold":corrg,"vs_hs300":corr_hs,
           "mmf_ann":None},open(f"{R}/out/metrics.json","w"),ensure_ascii=False,indent=1,default=str)
T.to_csv(f"{R}/out/monthly_panel.csv")
