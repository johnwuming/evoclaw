...
## 恢复点续3
- G5 容量已复算（V3 全持仓口径）：中位 10509.2万、≥2000万 95.9%、缺数据月 3/97 → G5 PASS
- 剩余：IC 监控 + 死因核验（为何行业中性化推高 corr）→ 写 R-294
## 恢复点续4（死因核验）
- IC 监控：dual_low 全窗均值 0.0451 / price 0.0966（与 R-289 逐位一致）；roll6 未触发（dual_low 尾6=0.084,0.265,-0.289,-0.135,-0.059,0.245）
- 下一步：核验 V3 行业中性化是否机械生效（行业集中度是否下降）、"其他"桶占比（截面太小→过拟合）、残余 corr 驱动
## 死因核验结论（机制）
- V3 行业中性化**机械生效**：持仓行业集中度下降（中位最大行业权重 0.138 vs V2 0.167；HHI 0.084 vs 0.09；行业数 15 vs 16）
- 但 corr 反而上升 0.583→0.622：**行业残差化剥掉行业层差异后，选券更集中于「行业内低价/小盘」维度，而这恰是与微盘引擎共振的维度**（R-289 §6 根因：低价转债与微盘共同驱动=流动性溢价/小市值，不单纯由行业介导）
- 即：行业中性化把「行业选择的自由度」让渡给「价格/规模维度」，后者才是 corr 源头 → 去相关方向反了
- V5（降权低价腿 0.5P+0.5D）corr 0.599 仍>0.5 → 低价腿权重 0.7→0.5 只降 0.08，不够；完全砍 P 腿不可行（超额全靠 P）
- 结论：**行业中性化路径失败**（G6 corr 0.622>0.5），替代=csad 线并行在跑（R-291 §3.3 先验风险亦提示 csad 与波动率族弱共振，需 E1 级策略层 corr 实测）
## 编号修正
- R-294 已被并行 csad 线占用（R-294-csad新引擎探索.md，task-0475）→ 本线执行报告改排 **R-295**（R-295-可转债轮动E2三次执行报告-正股行业中性化去相关.md），内部引用已同步更新
- 预注册 R-293（sha256 fb785665）不受影响；E2 执行报告 = R-295
