...
## 待办
- [ ] 对账：w_eng(prev_me) vs shadow_nav w_applied 全表一致率
- [ ] 污染月名单：prev_me 是日历NaN 且 w_true>0 的账本行
- [ ] 反事实净值：w_true 全路径替换，终点差 + MDD 差
- [ ] shadow_ledger 侧 engines_shadow_nav_gold.py 是否同一 compute_signals（grep）
- [ ] 上游 R-380 42% gold 腿数据源确认
