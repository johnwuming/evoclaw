...
- `index_option_1000index_qvix()`：2791 行 2015-02-09~2026-08-17，与 50ETF QVIX 完全同长度同起点 → **疑似同源/回填，待验证**（中证1000期权 2022-07 才上市，2015-2022 数据必为回算，标注不可直接采信）。
- `stock_margin_szse("20260807")` 实测 OK：单日 1 行 6 列（融资买入额/融资余额/融券卖出量/融券余量/融券余额/融资融券余额）。历史抓取需逐日调用（~4000 次），可选增强；基础版用 SSE 批量接口即可（SSE 2010-03 起，占全市场两融大部分）。
- NMTAP 精确出处（知乎六月金工精选摘要，2026-08-18 智谱 reader 抓取成功）：华福《市场情绪指标专题（三）：情绪和市场底部：净融入交易额占比（NMTAP）》2022-06-16；NMTAP 基于两融业务，行为金融基础=归因偏差+过度自信，有长假效应，2013 年以来万得全A 中期底部/市场大底覆盖率优异。**原始公式付费墙未公开 → 我们的构造为自构造近似，报告标注**。
- PCR 出处确认：华福《市场情绪指标专题（二）：顶部和底部信号：期权看跌看涨比（PCR）》2022-06-01，PCR=看跌/看涨**持仓量**比，对中期顶部提示效果更佳。
- 主力净流入率（华福底部信号 B2/顶部 T3 用）：无免费逐笔/大单数据源，**放弃直接实现，降级为量价代理**（全量池成交额结构），与 R-227 T2 判断一致。

## 2. 设计结论要点（报告素材，详见 R-230）

- 五维 = D1 MA15（已有 v6a_def）+ D2 广度 + D3 资金（NMTAP 可得/主力放弃→代理）+ D4 情绪（PCR 2015+/QVIX 2015+）+ D5 超跌危机（自算）。
- 优先级结构采用华福式覆盖（底部/危机 > 顶部 > 均线），与现役乘法合成做 A/B（E2）。
- 回测缺口纪律：NMTAP 2010 前、PCR 2015 前恒"不触发"，分段披露，禁止回填。
- n_trials 预算：E1 信号画像 ~40 组 + E2 结构 A/B 2 组 + E3 增量 ~12 组，全部入 ledger；扩散阈值遍历看平台不挑峰（华福自曝 0.85 无平台）。
