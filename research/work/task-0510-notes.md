...
- **分级：部分可行**。推翻 R-299「免费源完全缺」的隐含前提：PIT 余额可免费重建到季度粒度；坐实「月度粒度 + 全市场一次性建库」不适合自建自爬（工程缺口）。
- 缺口形态：①粒度季频非月频；②老券需深翻页，若用 cninfo searchkey+seDate 可解决但未实测；③清洁规则（同名多期转债、标题变体）需另建；④集思录全量当前快照需登录 cookie（30条墙）。
- PIT 质量：公告级最高（披露日=事实日）；集思录/东财快照仅实时，无历史价值。
