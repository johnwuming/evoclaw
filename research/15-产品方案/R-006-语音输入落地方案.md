...
## 附录 A: GLM-ASR 技术详解（来自 voice-input-plan）

### GLM-ASR 关键优势

1. **上下文智能理解** — 基于语言模型上下文优化输出，同音字纠错、标点自动添加
2. **热词支持** — `hotwords=["multi-agent", "OpenClaw", "GLM-5", "智谱"]`
3. **方言支持** — 8 种中国方言覆盖
4. **流式转录** — 支持 `stream: true`，未来可扩展实时体验

### OGG 转码

Telegram 语音默认 Opus in OGG，GLM-ASR 只支持 wav/mp3，需 ffmpeg 转码：
```python
subprocess.run(["ffmpeg", "-i", ogg_path, "-ar", "16000", "-ac", "1", wav_path])
```

### GLM-ASR 限制
- 音频 ≤ 30 秒（Telegram 语音通常 <60秒）
- 仅支持 wav/mp3（需 ffmpeg 转码）
- 依赖网络（本地 whisper 兜底）

---

## 附录 B: ASR 方案详细对比（来自 local-asr-comparison）

### 推荐方案：SenseVoice-Small + whisper.cpp 双链路

| 方案 | 中文准确率 | 速度 | 模型大小 |
|------|-----------|------|---------|
| **SenseVoice-Small** | ⭐⭐⭐⭐⭐ | 70ms/10秒(最快) | ~234M |
| **GLM-ASR-Nano** | ⭐⭐⭐⭐⭐ | 需GPU | 1.5B |
| **whisper.cpp large-v3** | ⭐⭐⭐⭐ | 10-30秒 | ~1.5G |
| **faster-whisper** | ⭐⭐⭐⭐ | 5-15秒 | ~1.5G |
| **FunASR Paraformer** | ⭐⭐⭐⭐ | 快 | ~220M |

SenseVoice-Small 优势：中文准确率超过 Whisper-Large，速度比 Whisper-Large 快 15 倍，支持语音情感识别。
